...
function* submitAddReviewForm(payload, dentistId) {
  try {
    ...
    if (errors.rating) {
      yield put(toastrActions.error('', 'Rating is required. Click on the teeth to rate.'));
    } else {
      yield put(toastrActions.error('', 'Please fix errors on the form!'));
    }
...
function* submitPaymentFormWatcher() {
  while (true) {
    const { payload, user } = yield take(SUBMIT_PAYMENT_FORM);

    try {
      const stripeToken = yield call(makeStripeCreateTokenRequest, payload);
      if (stripeToken) {
        const requestURL = `/api/v1/users/${user.id}/account/payment/sources/${stripeToken}/`;
        const params = {
          method: 'POST',
